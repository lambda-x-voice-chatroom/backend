...
router.post('/attachSourceToCustomer', async (req, res) => {
    try {
        ...
        const newlyAttachedSource = await stripe.customers.createSource(
            userStripeId,
            { source: sourceId }
        );
...
router.post('/updateDefaultSource', async (req, res) => {
    try {
        ...
        const newlyUpdatedSource = await stripe.customers.update(userStripeId, {
            source: sourceId
        });
...
router.post('/groupTwilioCharges', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            sumOfGroupTwilioCharges: sumOfGroupTwilioCharges
        });
    } catch (err) {
        // console.log(err);
        res.status(500).json(err);
...
router.get('/allTwilioCharges', async (req, res) => {
    try {
        ...
        const allTwilioChargesRes = await client.calls.list();
        console.log(
            'allTwilioChargesRes in /allTwilioCharges: ',
            allTwilioChargesRes
        );
...
router.post('/userStripeCharges', async (req, res) => {
    try {
        ...
        let userStripeChargesArr = allChargesRes.data.filter(
            charge => charge.customer === userStripeId
        );
        ...
        res.status(200).json({
            sumOfUserStripeCharges: sumOfUserStripeCharges
        });
    } catch (err) {
        // console.log('err: ', err);
        res.status(500).json(err);
...
router.post('/updateCreditCard', async (req, res) => {
    try {
        ...
        res.status(200).json({ updatedSource: response });
    } catch (err) {
        // console.log('err: ', err);
        return err;
    }
...
router.post('/addMoney', async (req, res) => {
    ...
    try {
        ...
        const customerStripeInfo = await axios.post(
            `${host}/api/billing/retrieveCustomerDefaultSource`,
        ...
        if (chargeResponse.data.charge.status === 'succeeded') {
            ...
            const sumOfUserStripeChargesRes = await axios.post(
                `${host}/api/billing/userStripeCharges`,
                { userStripeId: userStripeId }
            );
            console.log(
                'sumOfUserStripeChargesRes :',
                sumOfUserStripeChargesRes
            );

            let sumOfUserStripeCharges =
                sumOfUserStripeChargesRes.data.sumOfUserStripeCharges;
            console.log(
                'sumOfUserStripeCharges [dollars]: ',
                sumOfUserStripeCharges
            ); //in dollars

            //get the sum of all the user's twilio charges
            const allTwilioChargesRes = await axios.get(
                `${host}/api/billing/allTwilioCharges`
            );
            console.log(
                'allTwilioChargesRes in /addMoney endpoint: ',
                allTwilioChargesRes
            );
            ...
            const userOwnedGroupsRes = await axios.get(
                `${host}/api/users/${userId}/groupsOwned`
            );
            ...
            for (let i = 0; i < userOwnedGroupsIds.length; i++) {
                console.log(
                    "start of adding each group's twilio charges to the twilioChargesForEachUserOwnedGroup array"
                );
                let userOwnedGroupId = userOwnedGroupsIds[i];
                // console.log('userOwnedGroupId: ', userOwnedGroupId);
                const groupTwilioChargesRes = await axios.post(
                    `${host}/api/billing/groupTwilioCharges`,
                    { groupId: userOwnedGroupId }
                );
                console.log('groupTwilioChargesRes: ', groupTwilioChargesRes);
                console.log(
                    'groupTwilioChargesRes.data.sumOfGroupTwilioCharges: ',
                    groupTwilioChargesRes.data.sumOfGroupTwilioCharges
                );

                const sumOfGroupTwilioCharges =
                    groupTwilioChargesRes.data.sumOfGroupTwilioCharges;
                console.log(
                    'sumOfGroupTwilioCharges: ',
                    sumOfGroupTwilioCharges
                );

                twilioChargesForEachUserOwnedGroup.push(
                    sumOfGroupTwilioCharges
                );
                console.log(
                    'twilioChargesForEachUserOwnedGroup: ',
                    twilioChargesForEachUserOwnedGroup
                );
            }
            console.log(
            ...
            const sumOfUserTwilioCharges = twilioChargesForEachUserOwnedGroup.reduce(
                (acc, cur) => acc + cur,
                0
            );
            ...
            console.log(
                'sumOfUserTwilioCharges (exact): ',
                sumOfUserTwilioCharges
            );
            ...
            const updatedAccountBalance =
                sumOfUserStripeCharges + sumOfUserTwilioCharges;
            console.log('updatedAccountBalance: ', updatedAccountBalance);
            console.log(
                'updatedAccountBalance type: ',
                typeof updatedAccountBalance
            );

            await axios.put(`${host}/api/users/${userId}/accountBalance`, {
